...
eb=parseInt(t.offsetWidth)-parseInt(lb.offsetWidth);lb.style.left=Math.round(this.getValue()*eb/l.L5)+ub.i;lb.style.top=Math.round((l.kW-l.KP)/2)+ub.i;}else{var
u=parseInt(t.offsetHeight)-parseInt(lb.offsetHeight);lb.style.left=Math.round((l.kW-l.KP)/2)+ub.i;lb.style.top=u-Math.round(this.getValue()*u/l.L5)+ub.i;}}};r.Hj=function(f,c,h){var
...
W=this.getOrientation()==0;if(!g.boxtype)g.boxtype=I?ub.l:ub.m;g.tagname=ub.n;if(g.left==null&&Q!=null)g.left=Q;if(g.top==null&&Fb!=null)g.top=Fb;if(W){g.height=l.kW;g.width=this.getLength();}else{g.height=this.getLength();g.width=l.kW;}var
...
P=Math.round((l.kW-l.M6)/2)+ub.r;var
...
ob=W?Math.round(this.getValue()/l.L5*(Ka.ee()-l.rI)):Math.floor((l.kW-l.KP)/2);var
ja=W?Math.floor((l.kW-l.KP)/2):Math.round(Ka.ld()-l.rI-this.getValue()/l.L5*(Ka.ld()-l.rI));var
Db=W?l.rI:l.KP;var
...
F=this.getTrackClickable()&&t?this.ri(ub.z,ub.A,2):ub.o;var
E=v.lg(0);E.setStyles(this.ch()+this.ke()+Eb);E.setAttributes(ub.B+F);var
ba=this.getHandleImage();if(ba==null)ba=na?l.tT:l.HV;var
_a=D.lg(1);_a.setAttributes(ub.C+(t?this.ri(ub.D,ub.E,2):ub.o));var
Cb=_a.lg(0);Cb.setAttributes(ub.F+(t?ub.o:ub.G)+ub.H+jsx3.html.Mf);Cb.setStyles(ub.I+(this.getUriResolver()).resolveURI(ba)+ub.J);return (D.paint()).join((v.paint()).join((E.paint()).join(ub.K)+(_a.paint()).join((Cb.paint()).join(ub.o))));};r.sx=function(k,o){if(!k.leftButton())return;k.cancelBubble();k.cancelReturn();var
