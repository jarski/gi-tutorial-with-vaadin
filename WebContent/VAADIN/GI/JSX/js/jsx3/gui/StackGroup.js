...
ub={a:"27",b:"*",c:"box",d:"100%",e:"auto",f:",",g:"beforeEnd",h:"3.0.00"};var
jb=jsx3.gui.LayoutGrid;c.ORIENTATIONV=0;c.ORIENTATIONH=1;c.DEFAULTSTACKCOUNT=2;c.DEFAULTDIMENSIONS=[ub.a,ub.b];c.DEFAULTBARSIZE=27;p.init=function(q){this.jsxsuper(q);};p.getBarSize=function(){return this.jsxbarsize;};p.setBarSize=function(s){this.jsxbarsize=s;return this;};p.Id=function(g,m){if(m==null)m=g.getChildIndex();var
xb=this.Gi(m);if(xb)return xb;var
Ha=(this.getParent()).Id(this);var
...
N=0,Ba=Aa,ca=nb,C=0;if(gb){C=Fb;ca=g.isFront()?nb-Ga*((this.getChildren()).length-1):Ga;}else{N=Fb;Ba=g.isFront()?Aa-Ga*((this.getChildren()).length-1):Ga;}return this.Ei(m,{boxtype:ub.c,left:N,top:C,width:ub.d,height:ub.d,parentwidth:Ba,parentheight:ca});};p.Hj=function(m,j,e){var
...
Ya=this.Id(F,Ba);e.add(F,Ya,j!=null,true);}};p.paint=function(){var
...
gb=this.getRendered();if(gb!=null){jsx3.html.insertAdjacentHTML(gb,ub.g,d.paint());jsx3.gui.Painted.prototype.paintChild.call(this,d,k,gb,true);}if(!k)this.A0(true);};p.onSetChild=function(q){return q instanceof jsx3.gui.Stack;};p.onRemoveChild=function(g,m){this.jsxsuper(g,m);if(jsx3.$A.is(g)){this.repaint();}else{var
