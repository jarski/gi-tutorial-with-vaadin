...
ob=[],Pa=[];for(var
wa=0;wa<T.length;wa++)if(Ja.Menu&&T[wa] instanceof Ja.Menu||Ja.ToolbarButton&&T[wa] instanceof Ja.ToolbarButton){Pa.push(T[wa]);}else if(ob.length==0){T[wa]=this.Xr(T[wa],this.isFront());ob.push(T[wa]);}var
Q=this.paintChildren(Pa);var
sb=this.paintChildren(ob);var
ha=Lb.lg(0);var
Wa=this.getOrientation()==0?ub.A:ub.B;ha.setAttributes(ub.x+Wa+ub.w+jsx3.html.Mf);var
Ib=(ha.paint()).join(this.il());return (u.paint()).join((Lb.paint()).join(Ib+Q)+sb);};r.Xr=function(h,d){var
